...
function Forgot() {
    ...
    const formik = useFormik({
        ...
        onSubmit: (values) => {
            ...
            .then((res) => res.json())
            .then((users)=>{
                const response = users.find((user) => user.email === values.email)
                if(response) {
                    formik.resetForm();
                    navigate('/confirm');  
                }
            ...
            .catch((err) => toast.error('Email not found'))
                        ...
                        <div className='w-full flex flex-col font-normal text-lg'>
                            <form className='w-full flex flex-col my-6 px-9' onSubmit={formik.handleSubmit}>
                                ...
                                <Link to='/confirm' className='back w-3/4 self-center flex items-center justify-center rounded-sm'>
                                    <button type='submit' className='w-full p-3 border-0 text-lg text-white font-bold font-sans'>Confirm</button>
                                </Link>
                            </form>
